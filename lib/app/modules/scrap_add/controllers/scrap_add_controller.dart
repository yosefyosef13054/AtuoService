...
import 'package:autoservice/app/modules/scrap_add/models/model.dart';
import 'dart:convert';
...
class ScrapAddController extends GetxController {
  //TODO: Implement ScrapAddController
...
//Car  Condition Lists
  ...
  List<CarStatus> listmachineStatus = [];
  List<CarStatus> listgearsStatus = [];
  List<CarStatus> listtireWheelStatus = [];
  List<CarStatus> listcarTransferType = [];
  var carStatus = '0'.obs;
  var machineStatus = '0'.obs;
  var gearsStatus = '0'.obs;
  var tireWheelStatus = '0'.obs;
  var carTransferType = '0'.obs;
  void getcarConditionLists() async {
    try {
      loading.value = true;
      var response = await http.get('Mobile/GetScrapDefaultList');
      DefaultLists defaultLists = DefaultLists.fromJson(response.data);
      listcarStatus = defaultLists.carStatus;
      listmachineStatus = defaultLists.machineStatus;
      listgearsStatus = defaultLists.gearsStatus;
      listtireWheelStatus = defaultLists.tireWheelStatus;
      listcarTransferType = defaultLists.carTransferType;
    } catch (e) {
      // print(e.response.data);
    }
  ...
  saveImages() async {
    try {
      ...
      var save = await http.postUrlUpload(
          'ServiceScrapCar/UploadFiles', formData,
...
//Create Add Request
  void submit(context) async {
    ...
    if (image.length < 1) {
      ...
      btnController.error();
      Future.delayed(Duration(milliseconds: 1200), () => btnController.reset());
    } else if (carTransferType.value == '0') {
      Fluttertoast.showToast(
          msg: 'selectTransfer'.tr,
          toastLength: Toast.LENGTH_LONG,
          gravity: ToastGravity.TOP,
          timeInSecForIosWeb: 0,
          backgroundColor: Colors.red,
      ...
      await saveImages();
      http.postUrl('ServiceScrapCar/Add', {
        ...
        'carTransferTypeId': carTransferType.value.toString(),
        // 'gearsStatusId': gearsStatus.value.toString(),
        // 'machineStatusId': machineStatus.value.toString(),
        // 'outsideStatusId': carStatus.value.toString(),
        // 'tiresStatusId': tireWheelStatus.value.toString(),
