...
class UsedCarsAddController extends GetxController {
  //TODO: Implement UsedCarsAddController
  ...
  saveImages() async {
    try {
      ...
      var save = await http.postUrlUpload(
          'ServiceUsedCar/UploadFiles', formData,
...
//default Lists
//Car  Condition Lists
  List<CarCheckType> listcarStatus = [];
  List<CarCheckType> listinsideStatus = [];
  List<CarCheckType> listelectricityStatus = [];
  List<CarCheckType> listtiresStatus = [];
  List<CarCheckType> listcarCheckType = [];
  var carStatus = '0'.obs;
  var insideStatus = '0'.obs;
  var electricityStatus = '0'.obs;
  var tiresStatus = '0'.obs;
  var carCheckType = '0'.obs;
  void getcarConditionLists() async {
    try {
      loading.value = true;
      var response = await http.get('Mobile/GetUsedCarDefaultList');
      CarConditionLists defaultLists =
          CarConditionLists.fromJson(response.data);
      listcarStatus = [];
      listinsideStatus = [];
      ...
      listcarCheckType = [];
      // listcarStatus
      //     .add(CarCheckType(id: 0, titleAr: 'select'.tr, titleEn: 'Choose'));
      // listinsideStatus
      //     .add(CarCheckType(id: 0, titleAr: 'select'.tr, titleEn: 'Choose'));
      // listelectricityStatus
      //     .add(CarCheckType(id: 0, titleAr: 'select'.tr, titleEn: 'Choose'));
      ...
      listcarStatus.addAll(defaultLists.carStatus);
      listinsideStatus.addAll(defaultLists.insideStatus);
      listelectricityStatus.addAll(defaultLists.electricityStatus);
      listtiresStatus.addAll(defaultLists.tiresStatus);
      listcarCheckType.addAll(defaultLists.carCheckType);
    } catch (e) {
      // print(e.response.data);
    }
  ...
  void submit(context) async {
    if (image.length < 1) {
      ...
      btnController.error();
      Future.delayed(Duration(milliseconds: 1200), () => btnController.reset());
    } else if (carCheckType.value == '0') {
      Fluttertoast.showToast(
          msg: 'conditionvalid'.tr,
          toastLength: Toast.LENGTH_LONG,
          gravity: ToastGravity.TOP,
          timeInSecForIosWeb: 0,
          backgroundColor: Colors.red,
      ...
      http.postUrl('ServiceUsedCar/Add', {
        ...
        'carCheckId': carCheckType.value.toString(),
        // 'electricityStatusId': electricityStatus.value.toString(),
        // 'insideStatusId': insideStatus.value.toString(),
        // 'outsideStatusId': carStatus.value.toString(),
        // 'tiresStatusId': tiresStatus.value.toString(),
        ...
        'kmCounter': kmController.text.toString()
      }).then((response) {
        // print(response);
        try {
          Future.delayed(Duration(milliseconds: 10), () {
