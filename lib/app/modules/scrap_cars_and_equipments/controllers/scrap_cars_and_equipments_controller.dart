...
class ScrapCarsAndEquipmentsController extends GetxController {
  ...
  void getusedCarList({regionId, carTypeId, modelId, yearId, areaId}) async {
    ...
    try {
      var response = await http.get(
          'ServiceScrapCar/Search?regionId=${regionId.toString()}&carTypeId=${cartypeId.toString()}&modelId=${modelId.toString()}&yearId=${yearId.toString()}&areaId=${governorateid.value}&pageIndex=0&pageSize=20');
      // print(response.data);
      ...
      for (var item in response.data) {
        ...
        usedCarList.contains(UsedCar(
                  ...
                  carModelName: item['carModelName'].toString(),
                  ...
                  carTransferTypeId: item['carTransferTypeId'],
                  carTransferTypeName: item['carTransferTypeName'],
                  gearsStatusId: item['gearsStatusId'],
                  gearsStatusName: item['gearsStatusName'],
                  machineStatusId: item['machineStatusId'],
...
//Car Items
  ...
  void itemsPagination(
    ...
    try {
      ...
      var response = await http.get(
          'ServiceScrapCar/Search?regionId=${regionId.toString()}&carTypeId=${cartypeId.toString()}&modelId=${modelId.toString()}&yearId=${yearId.toString()}&areaId=${area.toString()}&pageIndex=${index.toString()}&pageSize=20');
      // print(response.data);
      for (var item in response.data) {
        List<ImageModel> imagesModel = [];
